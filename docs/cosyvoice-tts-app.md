...
- **性能（官方标称，M2 Max）**: RTF ≈ 0.5（快于实时）；LLM ~13 ms/token、DiT 370–520 ms、HiFi-GAN 50–170 ms。CAM++ 说话人编码器走 CoreML/Neural Engine（FP16）。**本项目最新实测（Release 构建 + 预热 + 显存治理后）：RTF ≈ 0.45（2026-09-16）**。
...
2. **性能数据是官方标称**：RTF 0.5 为官方口径。本项目 M0 首测 0.88（Debug 未优化），最新实测 Release 构建 + 预热后 **RTF ≈ 0.45**（2026-09-16），优于官方标称。
