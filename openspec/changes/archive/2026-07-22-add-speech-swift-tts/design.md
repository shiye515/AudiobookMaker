...
固定 speech-swift 的已验证 release tag 和解析后的 revision，只 vendor 并链接 `CosyVoiceTTS`、`Qwen3TTS`、`AudioCommon`、`MLXCommon` 及其必要传递依赖；vendor 目录记录上游 revision/子树对象 ID 与唯一的 Intel compile-only 补丁。该补丁不改变 arm64 推理实现，只为上游可选 CAM++ 克隆组件提供 x86_64 不可用 stub，以使现有 universal App 保留 Intel Kokoro slice。通过一个可注入的 `SpeechSwiftPlatformSupport` 同时检查进程架构、Rosetta 翻译状态、系统版本与 Metal 可用性；只有全部满足时才将两个模型发布到目录并构造运行时。
...
把 `TTSModelCatalog.kokoro` 单例演进为按 ID 索引的签名 manifest 集合。每份清单包含平台约束、模型/变体 ID、speech-swift runtime revision、总下载/安装大小、许可、必需路径、允许的重定向来源，以及一个或多个带相对路径、URL、大小和 SHA-256 的 artifact。Kokoro 可继续使用单一归档 artifact；Hugging Face speech-swift 模型使用逐文件 snapshot artifacts。下载器只接受每份清单声明的 HTTPS origin/redirect host，并把所有模型安装到 `Application Support/.../Models/<encoded-id>/<version>`。

speech-swift session 必须从已验证的本地 URL 初始化；任何 `fromPretrained()` 的默认联网行为都通过显式本地 cache 路径和 `offlineMode` 关闭。安装器去除当前硬编码的 Kokoro package root：归档 artifact 按 manifest 声明的 archive root 安全解包，snapshot artifacts 按签名相对路径分别写入 staging；所有 artifact 全部校验后才原子提交。活动下载、resume data、进度与取消状态按 model ID 和 artifact 隔离，允许目录展示多个模型但同一时间只执行一个大模型安装。
...
Apple Silicon 实机验收分别覆盖两个模型的下载、冷/热加载、固定试听样句、中文 EPUB 中固定的前段/中段/后段代表性章节子集、独立长章节压力片段、暂停/取消、M4B、峰值内存、RTF 和清理；不要求把整本 EPUB 全部合成。另以 Intel 原生进程与 Apple Silicon Rosetta 进程验证模型不可见、无下载且不加载 MLX。自动测试不得替代中文听感验收。
