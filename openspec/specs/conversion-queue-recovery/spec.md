...
系统 SHALL 为用户启动的书籍创建持久化 ConversionJob，按 `queueOrdinal` 以 FIFO 顺序调度，并 SHALL 将任务创建时的模型 ID 固定到该任务。

#### Scenario: 启动一本就绪书籍
- **WHEN** 用户对就绪书籍执行“开始转换”
- **THEN** 系统创建排队任务并按 FIFO 顺序等待运行

#### Scenario: 多本书排队
- **WHEN** 活跃任务已达到并发上限且用户启动另一书籍
- **THEN** 新任务保持 queued 并显示其队列状态
...
系统 SHALL 在暂停时停止创建新请求、取消可安全取消的在途片段、保存 checkpoint，并 SHALL 在继续时从首个未验证完成片段恢复。
...
#### Scenario: 继续暂停任务
- **WHEN** 用户继续一个 paused 或 interrupted 任务
- **THEN** 系统重新验证运行时与模型并从首个未完成 checkpoint 开始
