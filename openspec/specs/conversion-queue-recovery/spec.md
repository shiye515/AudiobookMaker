...
系统 SHALL 为用户启动的书籍创建持久化 ConversionJob，按 `queueOrdinal` 以 FIFO 顺序调度，并 SHALL 将任务创建时的模型 ID、模型版本和音色 ID 固定到该任务，无论目标运行时是 Apple 系统语音、Kokoro、CosyVoice3 或 Qwen3-TTS。

#### Scenario: 使用 Kokoro 启动一本就绪书籍
- **WHEN** 用户以已安装 Kokoro 和有效音色对就绪书籍执行“开始转换”
- **THEN** 系统创建包含固定模型 ID、版本和音色 ID 的排队任务并按 FIFO 顺序等待运行

#### Scenario: 使用可下载模型启动一本就绪书籍
- **WHEN** 用户以已安装、平台兼容且具有有效音色的 Kokoro、CosyVoice3 或 Qwen3-TTS 对就绪书籍执行“开始转换”
- **THEN** 系统创建包含固定模型 ID、版本和音色 ID 的排队任务并按 FIFO 顺序等待运行

#### Scenario: 多本书排队
- **WHEN** 活跃任务已达到目标运行时并发上限且用户启动另一书籍
- **THEN** 新任务保持 queued、显示其队列状态且保留创建时的模型与音色三元组

### Requirement: 并发受用户设置与运行时能力共同限制
系统 SHALL 默认最多同时转换 1 本书，SHALL 允许用户将上限设为 1 或 2，且实际并发 MUST 不超过每个目标运行时建议上限；CosyVoice3 与 Qwen3-TTS 在未通过更高并发实机验收前 MUST 限制为 1。

#### Scenario: 用户设置高于运行时上限
- **WHEN** 用户设置并发 2 而目标运行时建议并发为 1
- **THEN** 系统实际只运行 1 个使用该运行时的任务并保持其余任务排队

#### Scenario: 两个高内存模型任务排队
- **WHEN** 一个 CosyVoice3 或 Qwen3-TTS 任务正在推理且另一高内存模型任务到达队首
- **THEN** 系统等待当前安全片段完成并释放运行资源后才加载下一个任务所需模型
...
系统 SHALL 在暂停时停止创建新请求、取消可安全取消的在途片段、保存 checkpoint，并 SHALL 在继续时验证任务锁定的模型 ID、模型版本、音色 ID 与平台兼容性，再从首个未验证完成片段恢复。

#### Scenario: 运行时支持即时取消
- **WHEN** 用户暂停正在合成的任务且运行时支持即时安全取消
- **THEN** 系统进入 pausing、取消在途请求并最终持久化为 paused

#### Scenario: 运行时不支持即时取消
- **WHEN** 用户暂停任务且 speech-swift 或其他运行时不能安全中止当前计算片段
- **THEN** 系统显示“完成当前片段后暂停”、停止创建后续请求并在当前片段边界持久化 paused

#### Scenario: 使用相同模型和音色继续任务
- **WHEN** 用户继续一个 paused 或 interrupted 任务且锁定的模型版本、音色与平台仍可用
- **THEN** 系统从首个未完成 checkpoint 开始并保持原模型、版本和音色

#### Scenario: 锁定的模型版本或音色不可用
- **WHEN** 用户继续任务但其模型版本缺失、损坏或 voice ID 不存在
- **THEN** 系统保持任务暂停并引导恢复相同资源，MUST NOT 静默替换模型或音色

#### Scenario: 锁定的模型版本、音色或平台不可用
- **WHEN** 用户继续任务但其模型版本缺失或损坏、voice ID 不存在，或者 speech-swift 模型不受当前运行环境支持
- **THEN** 系统保持任务暂停并引导恢复相同资源或在兼容 Apple Silicon 环境继续，MUST NOT 静默替换模型或音色
